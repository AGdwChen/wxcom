  ...
  methods: {
    ...
    singleSlip(e) {
      ...
      if (state.touchLast.x && state.touchLast.y) {
        ...
        const move = (_x = false, _y = false) => {
          ...
          const param = {};
          if (_x) {
            let left = that.data.img.left + state.touchMove.x * movespeed;
            left = left > 0 ? 0 : left;
            left = left < (w - that.data.img.width) ? (w - that.data.img.width) : left;
            param.left = left;

            // if (right > w && that.data.img.left < 0) {
            //param.left = that.data.img.left + state.touchMove.x * movespeed
          ...
          if (_y) {
            let top = that.data.img.top + state.touchMove.y * movespeed;
            top = top > 0 ? 0 : top;
            top = top < (h - that.data.img.height) ? (h - that.data.img.height) : top;
            param.top = top;

            // if (bottom > h && that.data.img.top < 0) {
            //   param.top = that.data.img.top + state.touchMove.y * movespeed
    ...
    doubleSlip(e) {
      ...
      if (state.touchList[0].x && state.touchList[0].y) {
        let changeScale = (Math.sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0)) - Math.sqrt((state.touchList[1].x - state.touchList[0].x) * (state.touchList[1].x - state.touchList[0].x) + (state.touchList[1].y - state.touchList[0].y) * (state.touchList[1].y - state.touchList[0].y))) * speed;
    ...
    outputImg() {
      this.triggerEvent('selImg', {}) // 只会触发 pageEventListener2
    },
    getImgInfo(path) {
      return new Promise((resolve, reject) => {
        wx.getImageInfo({
    ...
    setImgPos({ width, height, top, left }) {
      ...
      top = top != undefined ? top : this.data.img.top;
      left = left != undefined ? left : this.data.img.left
      this.setData({
    ...
    getImg() {
      ...
      const saveImg = () => {
        ...
        wx.canvasToTempFilePath({
          ...
          success(res) {
            console.log(res);
            that.triggerEvent("setImg", { imgUrl: res.tempFilePath }, {});
          },
          fail(err) {
            console.log(err)
          }
